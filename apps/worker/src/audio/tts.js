...
/**
 * View a chunk of little-endian PCM16 as Int16 samples.
 *
 * Two things make the obvious `new Int16Array(buf.buffer, buf.byteOffset, ...)`
 * unsafe on streamed audio: Node pools small Buffers, so `Buffer.from(base64)`
 * can land on an ODD byteOffset, and Int16Array demands 2-byte alignment —
 * which throws, mid-call, only for some chunks. A chunk can also carry an odd
 * trailing byte when a sample straddles the boundary; it is dropped rather than
 * shifting every sample after it by one byte and turning the rest into noise.
 */
function asInt16(pcm) {
  const usable = pcm.length - (pcm.length % 2);
  if (pcm.byteOffset % 2 === 0) {
    return new Int16Array(pcm.buffer, pcm.byteOffset, usable / 2);
  }
  const aligned = new ArrayBuffer(usable);
  new Uint8Array(aligned).set(pcm.subarray(0, usable));
