...
import { DISCLOSURE } from '../src/lib/screening.js';
...
const validInvite = {
  valid: true,
  invite: { id: 1, reference_code: 'H7K294', status: 'active', expires_at: '2026-09-06T18:30:00Z', application_id: 44 },
  candidate: { id: 501, first_name: 'Ananya', cv_summary: 'Six years Node.js.' },
  job: { id: 12, title: 'Senior Backend Engineer' },
  criteria: [{ id: 1, criterion: 'Backend depth', weight: 5 }]
};

function harness(lookup = async () => validInvite) {
  const api = {
    lookupInvite: vi.fn(lookup),
    logUnmatched: vi.fn(async () => ({})),
    escalate: vi.fn(async () => ({})),
    updateCall: vi.fn(async () => ({})),
    startCall: vi.fn(async () => ({ id: 1 })),
    saveAnalysis: vi.fn(async () => ({})),
    saveTranscript: vi.fn(async () => ({}))
  };
  const sessions = new SessionManager();
  const session = sessions.create('call-1', { callerNumber: '+919000000000' });
  session.state = STATES.VERIFY_CODE;
  return { api, session, tools: new ToolExecutor(config, logger, api) };
}
...
describe('3.1 Pratibha never places an outbound call', () => {
  ...
  it('never references a call-creation endpoint', () => {
    const forbidden = [/api\.plivo\.com/i, /\/Call\//, /Calls?\.create/, /makeCall/i, /\bdial\(/i];
    for (const file of files) {
      const source = fs.readFileSync(file, 'utf8');
  ...
  it('holds no credential that could authenticate an outbound call', () => {
    // Plivo's REST API needs an auth id alongside the token. The service is
    // given only the token, which verifies webhooks and cannot place a call.
...
describe('3.2 AI disclosure', () => {
  ...
  it('is only marked given once Plivo confirms the audio played', () => {
    const source = fs.readFileSync(path.join(SRC, 'lib', 'callSession.js'), 'utf8');
    // The flag is set inside the playback-confirmation handler, not where we
    // queued the audio - "we tried to say it" is not the claim being made.
    ...
    // And nowhere else, or the guarantee is only as good as the intent.
    const occurrences = source.match(/aiDisclosureGiven = true/g) ?? [];
    expect(occurrences).toHaveLength(1);
  });
});

// 3.7 / 10 - "A caller without a valid, unexpired reference code cannot reach screening."
describe('3.7 the reference code gates the line', () => {
  it('has no transition into SCREEN that skips verification or consent', () => {
    expect(canTransition(STATES.GREET, STATES.SCREEN)).toBe(false);
    expect(canTransition(STATES.VERIFY_CODE, STATES.SCREEN)).toBe(false);
  ...
  it('leaves an unverified session failing the verification check', () => {
  ...
  it('refuses a tool that does not belong to the current state', async () => {
    ...
    expect(session.state).toBe(STATES.VERIFY_CODE);
  });

  it('does not advance or attach a candidate on an invalid code', async () => {
    const { tools, session, api } = harness(async () => ({ valid: false, reason: 'invalid_code' }));
    const result = await tools.execute('verify_code', { code: 'ZZZZZZ' }, session);

    expect(result.valid).toBe(false);
    expect(session.invite).toBeNull();
    expect(session.candidate).toBeNull();
    expect(session.state).toBe(STATES.VERIFY_CODE);
    expect(api.logUnmatched).toHaveBeenCalledOnce();
  });

  it('ends the call after two failed codes', async () => {
    const { tools, session } = harness(async () => ({ valid: false, reason: 'invalid_code' }));
    const first = await tools.execute('verify_code', { code: 'AAAAAA' }, session);
    expect(first.attempts_left).toBe(1);
    expect(session.ended).toBe(false);

    const second = await tools.execute('verify_code', { code: 'BBBBBB' }, session);
    expect(second.attempts_exhausted).toBe(true);
    expect(session.ended).toBe(true);
    expect(session.outcome).toBe(TERMINAL.WRONG_CODE);
  });

  it('refuses an expired invite, logs it, and raises it for a human', async () => {
    const { tools, session, api } = harness(async () => ({ valid: false, reason: 'expired_code' }));
    const result = await tools.execute('verify_code', { code: 'EXPIRD' }, session);

    expect(result.reason).toBe('expired');
    expect(session.invite).toBeNull();
    expect(session.outcome).toBe(TERMINAL.WRONG_CODE);
    expect(api.logUnmatched).toHaveBeenCalledOnce();
    expect(api.escalate).toHaveBeenCalledOnce();
  });
...
describe('8.2 "not a good time"', () => {
  it('ends with call_back_later and offers no callback', async () => {
    ...
    // Worded here rather than by the model, and it must put the onus on them
    // to call back - never suggest anyone will ring them.
    expect(result.say_exactly).toMatch(/call this same number back/i);
    expect(result.say_exactly).toMatch(/same reference code/i);
...
describe('8.6 escalation wording is fixed, not generated', () => {
  ...
  it.each(categories)('gives %s a verbatim line that promises only email', async (category) => {
    ...
    // Never a transfer, never a phone call back.
...
describe('3.5 scores must cite verifiable evidence', () => {
  let analyst; let api; let session;

  beforeEach(() => {
    api = { saveAnalysis: vi.fn(async () => ({})) };
    analyst = new PostCallAnalyst(config, logger, api);
    session = {
      id: 'call-1',
      osCallId: 7001,
      job: { title: 'Senior Backend Engineer', jd_summary: 'Backend' },
      candidate: { cv_summary: 'Six years Node.js' },
      criteria: [
        { id: 1, criterion: 'Backend depth', weight: 5 },
        { id: 2, criterion: 'Mentoring', weight: 3 }
      ],
      transcript: {
        entries: [
  ...
  const stub = (input) => {
    analyst.anthropic = {
      messages: { create: async () => ({ content: [{ type: 'tool_use', name: 'submit_assessment', input }] }) }
    };
  };
  ...
  it('drops a fabricated quote rather than filing it as evidence', async () => {
    ...
    // The criterion is reported unassessed, so a reviewer sees a gap rather
    // than silently seeing nothing at all.
  ...
  it('can only ever recommend, never reject', async () => {
    ...
    expect(api.saveAnalysis.mock.calls[0][1].sets_status).toBe('recommended');
  });

  it('never asks the API to set an application to rejected', () => {
...
describe('4.3 operating hours', () => {
  ...
  it('is open on a Tuesday afternoon IST', () => {
    // 2026-09-01 09:00 UTC = 14:30 IST, Tuesday
  ...
  it('is closed on Sunday', () => {
    // 2026-09-06 is a Sunday; 09:00 UTC = 14:30 IST
    expect(isWithinOperatingHours(config.operatingHours, at('2026-09-06T09:00:00Z'))).toBe(false);
  });

  // The shipped default: around the clock, but on weekdays only. 0 to 24 has to
  // mean "always open", not "never open", or every caller hears the closed
  // message on a day the line is meant to answer.
  ...
  // The window the code enforces and the sentence callers hear used to be two
  // independent settings. Changing one and forgetting the other sends every
  // out-of-hours caller back at a time the line will not answer.
...
// The dead-air filler exists so a silent model never leaves a live call in
// silence. Checking only the current recursion level made it fire after a
// perfectly good closing line, so Pratibha apologised for not catching a
// candidate who had already said goodbye.
describe('dead-air filler only covers real silence', () => {
  it('stays quiet when the turn already spoke before ending the call', async () => {
    ...
    // Turn 1: says goodbye, then calls end_call. Turn 2: nothing left to add.
    const replies = [
      { content: [
        { type: 'text', text: 'Thanks for your time, Ananya. The team will be in touch by email.' },
        { type: 'tool_use', id: 'tu1', name: 'end_call', input: { outcome: 'wrong_code' } }
...
describe('configuration is checked at boot, not on the first call', () => {
  ...
  it('names each missing credential', async () => {
    ...
    expect(problems.join(' ')).toMatch(/OS_SERVICE_TOKEN/);
  });

  it('catches signature verification switched on with no token', async () => {
    const { validateConfig } = await import('../src/config/index.js');
    // Fails closed, so this combination refuses every real call with a 403 and
    // the line looks dead rather than misconfigured.
  ...
  // Only the vendor that is actually going to speak has to be configured -
  // demanding both would make the switch useless, and demanding neither means
  // finding out on the first thing Pratibha tries to say.
  describe('the chosen voice vendor', () => {
    ...
    it('names the ElevenLabs credentials it needs, and only when it needs them', async () => {
      ...
      // Sarvam still does the listening, so its key is still required - but not
      // a second time for a voice it is no longer providing.
