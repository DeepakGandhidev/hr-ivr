 ...
 */
export function streamXml(config, callUUID) {
...
export function closedXml(config) {
  const message =
    `Thanks for calling ProMonkey Technologies. Our first round calls are answered ` +
    `${config.operatingHours.label}, India time. Please call back during those hours ` +
    `with your reference code, and we'll be glad to speak with you.`;
...
export function busyXml(config) {
  const message =
    `Thanks for calling ProMonkey Technologies. All our lines are busy at the moment. ` +
    `Please call back in a few minutes with your reference code.`;
